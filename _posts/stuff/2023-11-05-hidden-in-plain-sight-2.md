...
Since the expression _"adds"_ an integer and a string, the result is considered as a number with an implicit cast from string to integer. Another interesting thing is that an index expressed as a string would also be turned into an integer if such a value is expected and if possible. For instance:
...
_This is also possible with the XOR operation without the '+' symbol, but it would restrict the available numbers to those only made of 0 and 1: 10, 11, 100, etc._

However, we decided to not use the `'+'` symbol since it would be too easy, and to use only the numbers _0_ and _1_. To extract the letters `'a'` and `'y'` from `'Array'`, we therefore used a trick to extract the 11th character (index 10) of the string `'11ArrayArray'`:
